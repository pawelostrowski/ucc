...
void wprintw_utf(WINDOW *win_active, bool use_colors, short color_p, std::string buffer_str)
{
	...
	wattrset_color(win_active, use_colors, color_p);	// przywróć kolor wejściowy
	...
	if(buffer_str[buffer_str.size() - 1] == '\n')	// jeśli jest kod \n, pętla wykona się o 1 mniej niż wielkość bufora
		pos_buffer_end = buffer_str.size() - 1;
	else					// w przeciwnym razie pętla wykona się tyle razy, ile ma wielkość bufora (nie ma błędu, bo w pętli i = 0)
		pos_buffer_end = buffer_str.size();
