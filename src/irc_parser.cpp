...
//		}
		...
		else if(raw_parm[1] == "NOTICE")
		{
			...
			else
			{
				...
				case 458:
					raw_notice_458(ga, chan_parm, raw_parm);
					break;
...
*/
		...
		if(raw_parm[2].size() > 0 && raw_parm[2][0] == '#' && i - x + 3 < RAW_PARM_MAX)
		{
			// klucz nicka trzymany jest wielkimi literami, używany podczas aktualizacji wybranych flag
			std::string nick_mode_chan_key = raw_parm[i - x + 3];
			...
			if(raw_parm[3][i] == 'q')
			{
				if(raw_parm[3][s] == '+')
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xMAGENTA "* " + raw_parm[i - x + 3] + " jest teraz właścicielem pokoju " + raw_parm[2]
							+ " (ustawił" + a + " " + nick_mode + ").");
			...
			else if(raw_parm[3][i] == 'o')
			{
				...
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xMAGENTA "* " + raw_parm[i - x + 3] + " jest teraz superoperatorem pokoju " + raw_parm[2]
							+ " (ustawił" + a + " " + nick_mode + ").");
			...
			else if(raw_parm[3][i] == 'h')
			{
				...
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xMAGENTA "* " + raw_parm[i - x + 3] + " jest teraz operatorem pokoju " + raw_parm[2]
							+ " (ustawił" + a + " " + nick_mode + ").");
			...
			else if(raw_parm[3][i] == 'X')
			{
				...
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xMAGENTA "* " + raw_parm[i - x + 3] + " jest teraz moderatorem pokoju " + raw_parm[2]
							+ " (ustawił" + a + " " + nick_mode + ").");
			...
			else if(raw_parm[3][i] == 'b')
			{
				...
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xRED "* " + raw_parm[i - x + 3] + " otrzymuje bana w pokoju " + raw_parm[2]
							+ " (ustawił" + a + " " + nick_mode + ").");
				}
				...
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xWHITE "* " + raw_parm[i - x + 3] + " nie posiada już bana w pokoju " + raw_parm[2]
							+ " (ustawił" + a + " " + nick_mode + ").");
				}
			}
			...
			else if(raw_parm[3][i] == 'e')
			{
				...
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xMAGENTA "* " + raw_parm[i - x + 3] + " posiada teraz wyjątek od bana w pokoju " + raw_parm[2]
							+ " (ustawił " + nick_mode + ").");
				}
			...
			else if(raw_parm[3][i] == 'I')
			{
				...
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
							xWHITE "* " + raw_parm[i - x + 3] + " nie jest już na liście zaproszonych w pokoju " + raw_parm[2]
							+ " (ustawił" + a + " " + nick_mode + ").");
				}
			}

			else if(raw_parm[3][i] == 'm')
			{
				if(raw_parm[3][s] == '+')
				{
					win_buf_add_str(ga, chan_parm, raw_parm[2],
					...
					for(int j = 1; j < CHAN_MAX - 1; ++j)
					{
						if(chan_parm[j] && chan_parm[j]->nick_parm.find(nick_mode_key) != chan_parm[j]->nick_parm.end())
						{
							win_buf_add_str(ga, chan_parm, chan_parm[j]->channel,
									xMAGENTA "* " + raw_parm[2] + " jest teraz administratorem czata (ustawił(a) "
									+ nick_mode + ").");
						}
					}
				}
...
void raw_privmsg(struct global_args &ga, struct channel_irc *chan_parm[], std::string *raw_parm, std::string &buffer_irc_raw)
{
	...
	{
		win_buf_add_str(ga, chan_parm, raw_parm[2],
				xBOLD_ON xMAGENTA "* " + form_start + get_value_from_buf(buffer_irc_raw, ":", "!") + xNORMAL " " + action_me, act_type);
	}
...
void raw_396(struct global_args &ga, struct channel_irc *chan_parm[], std::string *raw_parm)
{
	...
	win_buf_add_str(ga, chan_parm, chan, xGREEN "* Twoim wyświetlanym hostem jest teraz " + raw_parm[3]);
...
{
	if(buffer_irc_raw.find(" :Cannot send to channel (no external messages)\n") != std::string::npos)
	{
		win_buf_add_str(ga, chan_parm, chan_parm[ga.current]->channel,
				xRED "* Nie można wysłać wiadomości do pokoju " + raw_parm[3] + " (nie przebywasz w nim).");
	}

	else if(buffer_irc_raw.find(" :Cannot send to channel (+m)\n") != std::string::npos)
	{
		win_buf_add_str(ga, chan_parm, raw_parm[3],
...
void raw_421(struct global_args &ga, struct channel_irc *chan_parm[], std::string *raw_parm, std::string &buffer_irc_raw)
{
	...
	if(buffer_irc_raw.find(" :This command has been disabled.\n") != std::string::npos)
	{
	...
	else if(buffer_irc_raw.find(" :Unknown command\n") != std::string::npos)
	{
...
{
	win_buf_add_str(ga, chan_parm, chan_parm[ga.current]->channel, xRED "* " + raw_parm[3] + " - brak wystarczającej liczby parametrów.");
}
...
void raw_817(struct global_args &ga, struct channel_irc *chan_parm[], std::string *raw_parm, std::string &buffer_irc_raw)
{
	...
	{
		win_buf_add_str(ga, chan_parm, raw_parm[3],
				time_unixtimestamp2local(raw_parm[4]) + xMAGENTA "* " + raw_parm[5] + xNORMAL " " + action_me, 1, false);
	}
...
void raw_notice(struct global_args &ga, struct channel_irc *chan_parm[], std::string *raw_parm, std::string &buffer_irc_raw)
{
	...
	// jeśli to zaproszenie do rozmowy prywatnej, komunikat skieruj do pokoju z tą rozmową (pojawia się po wysłaniu zaproszenia dla nicka)
		...
		&& buffer_irc_raw.find(":*** " +  raw_parm[4] + " invited " + raw_parm[6] + " into the channel\n") != std::string::npos)
	{
...
void raw_notice_109(struct global_args &ga, struct channel_irc *chan_parm[], std::string *raw_parm, std::string &buffer_irc_raw)
{
	win_buf_add_str(ga, chan_parm, raw_parm[4],
			"* " xBOLD_ON "-" xMAGENTA + get_value_from_buf(buffer_irc_raw, ":", "!") + xTERMC "-" xNORMAL " "
			+ get_value_from_buf(buffer_irc_raw, raw_parm[4] + " :", "\n"));
}
...
void raw_notice_152(struct global_args &ga, struct channel_irc *chan_parm[], std::string &buffer_irc_raw)
{
	...
	else if(buffer_irc_raw.find(" :end of offline senders list\n") != std::string::npos)
	{
		// feature
	}
	...
	{
		win_buf_add_str(ga, chan_parm, chan_parm[ga.current]->channel,
				xWHITE "* " + get_value_from_buf(buffer_irc_raw, ":", "!") + ": " + get_value_from_buf(buffer_irc_raw, " :152 :", "\n"));
	}
}
...
{
	win_buf_add_str(ga, chan_parm, chan_parm[ga.current]->channel,
			xWHITE "* " + get_value_from_buf(buffer_irc_raw, ":", "!") + ": " + raw_parm[4] + " - wartość wyłączona.");
}
...
{
	win_buf_add_str(ga, chan_parm, chan_parm[ga.current]->channel, "* " xBOLD_ON xMAGENTA + raw_parm[4] + xNORMAL " - nie ma takiego nicka na czacie.");
}
...
{
	win_buf_add_str(ga, chan_parm, chan_parm[ga.current]->channel, xRED "* " + raw_parm[4] + " - nieprawidłowa maska.");
}
...
*/
void raw_notice_458(struct global_args &ga, struct channel_irc *chan_parm[], std::string *raw_parm)
{
	// poprawić na rozróżnianie uprawnień

	win_buf_add_str(ga, chan_parm, raw_parm[4], xRED "* Nie można usunąć nienadanego uprawnienia dla " + raw_parm[6] + " (" + raw_parm[5] + ").");
}
...
{
	win_buf_add_str(ga, chan_parm, raw_parm[4], xRED "* Dostęp zabroniony, nie posiadasz wystarczających uprawnień w pokoju " + raw_parm[4]);
}
